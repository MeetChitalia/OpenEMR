...
class AclMain
{
    ...
    public static function zhAclCheck($user_id, $section_identifier)
    {
        ...
        $res_user_allowed_stmt  = sqlStatement($sql_user_acl, array($section_identifier, $user_id, 1));
        $res_user_allowed       = sqlFetchArray($res_user_allowed_stmt);
        $count_user_allowed     = $res_user_allowed['count'];

        $res_group_allowed_stmt = sqlStatement($sql_group_acl_allowed, $groups_sql_param);
        $res_group_allowed      = sqlFetchArray($res_group_allowed_stmt);
    ...
    public static function aclCheckForm($formdir, $user = '', $return_value = '')
    {
        ...
        if (!is_array($tmp) || !isset($tmp['aco_spec'])) {
            return false;
        }
