...
class TransmitProperties
{
    ...
    {
        $result = sqlStatement("select email from users where username=? ", [$_SESSION["authUser"]]);
        $provider_info = sqlFetchArray($result);
    ...
    {
        $result = sqlStatement("select name, street, city, state, postal_code, phone, fax, weno_id from facility where id = ?", [$_SESSION['facilityId'] ?? null]);
        $locid = sqlFetchArray($result);

        if (empty($locid['weno_id'])) {
            //if not in an encounter then get the first facility location id as default
            $result2 = sqlStatement("select name, street, city, state, postal_code, phone, fax, weno_id from facility order by id asc limit 1");
            $default_facility = sqlFetchArray($result2);
    ...
    private function getPatientInfo()
    {
        ...
        $missing = 0;
        $result = sqlStatement("select title, fname, lname, mname, street, state, city, email, phone_cell, postal_code, dob, sex, pid from patient_data where pid=?", [$_SESSION['pid']]);
    ...
    public function getProviderPassword()
    {
        ...
        $result = sqlStatement($sql, [$uid]);
        $prov_pass = sqlFetchArray($result);
    ...
    {
        $result = sqlStatement("select date, height, weight from form_vitals where pid = ? ORDER BY id DESC", [$_SESSION["pid"] ?? null]);
        $vitals = sqlFetchArray($result);
        return $vitals;
    }

    private function getSubscriber()
    {
        $result = sqlStatement("select subscriber_relationship from insurance_data where pid = ? and type = 'primary'", [$_SESSION['pid']]);
        $sql = sqlFetchArray($result);
    ...
    public function getPharmacy()
    {
        ...
        $result = sqlStatement($sql, [$_SESSION['pid'] ?? null]);
        $give = sqlFetchArray($result);
