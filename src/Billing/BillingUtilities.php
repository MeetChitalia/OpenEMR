...
class BillingUtilities
{
    ...
    public static function addBilling(
        ...
        // Handle ADORecordSet objects
        $count = 0;
        if (is_object($tmp) && method_exists($tmp, 'FetchRow')) {
            $row = $tmp->FetchRow();
    ...
    public static function updateClaim(
        ...
        if (!$newversion) {
            ...
            $row_result = sqlQuery($sql, $sqlBindArray);
            
            // Handle ADORecordSet objects
            $row = array();
            if (is_object($row_result) && method_exists($row_result, 'FetchRow')) {
                $row = $row_result->FetchRow();
            } else {
                $row = $row_result;
            }
        ...
        if ($newversion) {
             ...
             ****/
            sqlBeginTrans();
            $version_result = sqlQuery("SELECT IFNULL(MAX(version),0) + 1 AS increment FROM claims WHERE patient_id = ? AND encounter_id = ?", array($patient_id, $encounter_id));
            
            // Handle ADORecordSet objects
            $version = array();
            if (is_object($version_result) && method_exists($version_result, 'FetchRow')) {
                $version = $version_result->FetchRow();
            } else {
    ...
    public static function getInvoiceRefNumber()
    {
        ...
        // Handle ADORecordSet objects
        if (is_object($trow) && method_exists($trow, 'FetchRow')) {
            $row = $trow->FetchRow();
            $trow = $row ? $row : array();
        }
