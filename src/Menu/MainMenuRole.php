...
class MainMenuRole extends MenuRole
{
    ...
    public function __construct(EventDispatcherInterface $dispatcher)
    {
        ...
        // $this->menu_update_map["Inventory"] = "updateInventory"; // Disabled to use JSON config
    ...
    {
        // DEBUG: Add breakpoint here
        error_log("DEBUG: Starting menu loading in MainMenuRole::getMenu()");
        
        // Collect the selected menu of user
        $mainMenuRole = $this->getMenuRole();
        
        // DEBUG: Log the menu role
        error_log("DEBUG: Menu role: " . $mainMenuRole);

        // Load the selected menu
        if (preg_match("/.json$/", $mainMenuRole)) {
            // load custom menu (includes .json in id)
            $menu_file = $GLOBALS['OE_SITE_DIR'] . "/documents/custom_menus/" . $mainMenuRole;
            error_log("DEBUG: Loading custom menu from: " . $menu_file);
            $menu_parsed = json_decode(file_get_contents($menu_file));
        } else {
            // load a standardized menu (does not include .json in id)
            $menu_file = $GLOBALS['fileroot'] . "/interface/main/tabs/menu/menus/" . $mainMenuRole . ".json";
            error_log("DEBUG: Loading standard menu from: " . $menu_file);
            $menu_parsed = json_decode(file_get_contents($menu_file));
        }

        // if error, then die and report error
        if (!$menu_parsed) {
            error_log("DEBUG: JSON parsing failed with error: " . json_last_error());
            die("\nJSON ERROR: " . json_last_error());
        }
        
        // DEBUG: Log successful parsing
        error_log("DEBUG: Menu parsed successfully, items count: " . count($menu_parsed));
    ...
    // This creates menu entries for the custom inventory dashboard
    protected function updateInventory(&$menu_list)
    {
        // Point to the custom inventory index
        $menu_list->url = '/interface/custom_inventory/';
        $menu_list->target = 'adm';
        $menu_list->children = array(); // Clear any existing children
        
        // Ensure the menu item has the correct label
        if (!isset($menu_list->label)) {
            $menu_list->label = 'Inventory';
        }
        
        // Set ACL requirements for admin access only
        $menu_list->acl_req = array(array('admin', 'super'));
        $menu_list->requirement = 0;
    }
}
