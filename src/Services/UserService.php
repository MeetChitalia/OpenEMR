...
class UserService
{
    ...
    {
        // TODO: look at deserializing uuid with createResultRecordFromDatabaseResult here
        $res = sqlStatement("SELECT * FROM `users` WHERE `id` = ?", [$_SESSION['authUserID']]);
    ...
    public function getUserByUUID($uuid)
    {
        ...
        $res = sqlStatement("SELECT * FROM `users` WHERE `uuid` = ?", [$uuid]);
        $user = sqlFetchArray($res);
    ...
    {
        $res = sqlStatement("SELECT `id` FROM `users` WHERE BINARY `username` = ?", [$username]);
        $id = sqlFetchArray($res);
        if (!empty($id['id'])) {
            return $id['id'];
        } else {
            return false;
