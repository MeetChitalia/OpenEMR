...
class PatientService extends BaseService
{
    ...
    public function getFreshPid()
    {
        ...
        if (is_array($result) && isset($result['pid'])) {
            return $result['pid'] === null ? 1 : intval($result['pid']);
        } else {
            // Fallback if sqlQuery doesn't return expected format
            $result = sqlStatement("SELECT MAX(pid)+1 AS pid FROM patient_data");
            if ($result && $row = sqlFetchArray($result)) {
                return $row['pid'] === null ? 1 : intval($row['pid']);
            }
    ...
    public function getPatientPictureDocumentId($pid)
    {
        ...
        if (is_array($result) && !empty($result['id'])) {
            ...
            // Fallback if sqlQuery doesn't return expected format
            $result = sqlStatement($sql, array($GLOBALS['patient_photo_category_name'], $pid));
            if ($result && $row = sqlFetchArray($result)) {
                return $row['id'];
            }
    ...
    public function touchRecentPatientList(array $patient): void
    {
        ...
        $row = sqlFetchArray(sqlStatement($query, [$curUser['id']]));
        ...
        // Add the columns from list_options
        ...
        $recent_patient_count = $GLOBALS['recent_patient_count'] ?? 10; // Default to 10 if not set
        $rp = array_slice($rp, 0, $recent_patient_count);
        $_rp = serialize($rp);
    ...
    public function getRecentPatientList(int $user_id = 0): array
    {
        ...
        $res = sqlFetchArray(sqlStatement($sql, [$currUser['id']]));
