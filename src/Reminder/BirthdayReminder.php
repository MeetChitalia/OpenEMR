...
class BirthdayReminder
{
    ...
    public function isDisplayBirthdayAlert()
    {
        ...
        $result = sqlStatement($sql, array($this->pid));
        ...
        $today = date('m-d');
        $dobStr = is_array($res) && isset($res['DOB']) ? strtotime($res['DOB']) : false;
        if ($dobStr === false) {
            return false;
        }
        ...
        if (
            ...
            (
                ...
                $today >= $dobStr &&
                $today <= date('m-d', strtotime('+28 days', strtotime(is_array($res) && isset($res['DOB']) ? $res['DOB'] : '1970-01-01')))
            ) ||
            // on and after
            (
    ...
    private function isBirthdayAlertOff()
    {
        ...
        $result = sqlStatement($sql, array($this->pid, $this->user_id));
        $res = sqlFetchArray($result);
        //if there is result
