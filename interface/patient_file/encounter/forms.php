...
/**
 * This contains the encounter forms.
 *
 * @package   OpenEMR
...
if (!function_exists('ensureArray')) {
    function ensureArray($result) {
        ...
        if (is_object($result) && method_exists($result, 'FetchRow')) {
            // Check if it's a single row result (like from sqlQuery)
            if (!$result->EOF) {
                $row = $result->FetchRow();
                if ($row) {
                    return $row; // Return as associative array for single row
                }
            }
            
            // For multiple rows, return array of arrays
            $array = array();
...
if (!empty($_GET['attachid'])) {
    ...
    foreach ($attachid as $aid) {
        ...
        // sqlQuery returns a recordset, so we need to fetch the first row
        if ($tmp && !$tmp->EOF) {
            $tmp = $tmp->FetchRow();
        }
        if (!empty($tmp['count'])) {
...
function createFollowUpEncounter() {
    ...
    $encounterId = (!empty($result['parent_encounter_id'])) ? $result['parent_encounter_id'] : ($result['id'] ?? null);
    ?>
...
$encounter_date = !empty($dateres['date']) ? date("Y-m-d", strtotime($dateres["date"])) : date("Y-m-d");
...
<span class="font-weight-bold"><?php echo xlt("Document(s)"); ?>:</span>
    ...
    foreach ($docs_list as $doc_iter) {
        ...
        $note = '';
        if ($noteData && !$noteData->EOF) {
            $noteData = $noteData->FetchRow();
            $notes = array();
...
if (
    ...
    foreach ($result as $iter) {
        ...
        if (substr($formdir, 0, 3) == 'LBF') {
            ...
            if (!empty($lrow) && !empty($lrow['grp_aco_spec'])) {
