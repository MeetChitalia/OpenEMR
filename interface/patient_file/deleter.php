...
$silentDelete = !empty($_REQUEST['silent_delete']);

if ($silentDelete) {
    $GLOBALS['sql_string_no_show_screen'] = 1;
}
...
function row_table_exists($table)
{
    return (bool) sqlFetchArray(sqlStatement("SHOW TABLES LIKE ?", [$table]));
}

function row_delete_optional($table, $where)
{
    if (!row_table_exists($table)) {
        return;
    }
...
function delete_patient_chart($patient)
{
    ...
    row_delete_optional("marketplace_dispense_tracking", "pid = '" . add_escape_custom($patient) . "'");
    row_delete_optional("pos_receipts", "pid = '" . add_escape_custom($patient) . "'");
    row_delete_optional("pos_refunds", "pid = '" . add_escape_custom($patient) . "'");
    row_delete_optional("patient_credit_transactions", "pid = '" . add_escape_custom($patient) . "'");
    row_delete_optional("patient_credit_balance", "pid = '" . add_escape_custom($patient) . "'");
    row_delete_optional("daily_administer_tracking", "pid = '" . add_escape_custom($patient) . "'");
    row_delete_optional("pos_remaining_dispense", "pid = '" . add_escape_custom($patient) . "'");
    row_delete_optional("pos_transactions", "pid = '" . add_escape_custom($patient) . "'");
    row_delete_optional("patient_credit_transfers", "from_pid = '" . add_escape_custom($patient) . "' OR to_pid = '" . add_escape_custom($patient) . "'");
    row_delete_optional("pos_transfer_history", "source_pid = '" . add_escape_custom($patient) . "' OR target_pid = '" . add_escape_custom($patient) . "'");
    row_delete_optional("pos_transaction_void_audit", "pid = '" . add_escape_custom($patient) . "'");
    ...
    <div class="container mt-3">
        ...
        if (!empty($_POST['form_submit'])) {
            ...
            if ($patient) {
                ...
                $patientIds = array_filter(array_map('trim', explode(',', (string)$patient)), function ($value) {
                    return ctype_digit((string)$value) && (int)$value > 0;
                });
                foreach ($patientIds as $patientId) {
                    delete_patient_chart((int)$patientId);
                }
            } elseif ($encounterid) {
        ...
        <form method='post' name="deletefrm" action='deleter.php?patient=<?php echo attr_url($patient) ?>&encounterid=<?php echo attr_url($encounterid) ?>&formid=<?php echo attr_url($formid) ?>&issue=<?php echo attr_url($issue) ?>&document=<?php echo attr_url($document) ?>&payment=<?php echo attr_url($payment) ?>&billing=<?php echo attr_url($billing) ?>&transaction=<?php echo attr_url($transaction); ?>&silent_delete=<?php echo attr_url($silentDelete ? '1' : '0'); ?>&csrf_token_form=<?php echo attr_url(CsrfUtils::collectCsrfToken()); ?>'>
