...
$billing_view = ($tmp && is_array($tmp) && $tmp['authorized']) ? 0 : 1;
    ...
    <div class="table-responsive">
        <table class="table table-hover jumbotron py-4 mt-3">
            ...
            // then also limit the query to documents that are linked to the issue.
                $queryarr = array($pid);
                // Check if categories table exists and has the expected structure
                $table_exists = false;
                $table_check = @sqlQuery("SELECT 1 FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'categories'");
                if ($table_check && is_array($table_check)) {
                    $table_exists = true;
                }
                
                if ($table_exists) {
                    // Check if the categories table has the expected columns
                    $columns_check = @sqlQuery("SELECT 1 FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'categories' AND column_name = 'name'");
                    if ($columns_check && is_array($columns_check)) {
                ...
                "d.foreign_id = ? AND cd.document_id = d.id AND c.id = cd.category_id ";
                        ...
                        $query = "SELECT d.id, d.type, d.url, d.name as document_name, d.docdate, d.list_id, d.encounter_id, '' as name " .
                        "FROM documents AS d, categories_to_documents AS cd WHERE " .
                        "d.foreign_id = ? AND cd.document_id = d.id ";
                    }
                } else {
                    $query = "SELECT d.id, d.type, d.url, d.name as document_name, d.docdate, d.list_id, d.encounter_id, '' as name " .
                    "FROM documents AS d WHERE " .
                    "d.foreign_id = ? ";
                }
                if ($issue) {
                    $query .= "AND d.list_id = ? ";
