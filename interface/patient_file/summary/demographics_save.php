...
// Check if this was submitted from a modal
$is_modal = isset($_SERVER['HTTP_X_REQUESTED_WITH']) && $_SERVER['HTTP_X_REQUESTED_WITH'] === 'XMLHttpRequest';

// Debug: Log received data
error_log("Demographics save - POST data: " . print_r($_POST, true));

if (!isset($_POST["csrf_token_form"])) {
...
$newdata['employer_data'] = array(); // Initialize employer_data array
$alertmsg = ''; // Initialize alertmsg variable
...
if ($is_modal) {
    // Return JSON response for modal context
    header('Content-Type: application/json');
    if ($alertmsg) {
        echo json_encode(['success' => false, 'message' => $alertmsg]);
    } else {
        echo json_encode(['success' => true, 'pid' => $pid, 'message' => xl('Patient information updated successfully.')]);
    }
    exit;
} else {
    // Standard response for non-modal context
    if ($alertmsg) {
