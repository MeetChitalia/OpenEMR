...
function generate_receipt($patient_id, $encounter = 0)
{
 //REMEMBER the entire receipt is generated here, have to echo DOC type etc and closing tags to create a valid webpsge
    ...
    $ferow = ensureArray($ferow);
    if (empty($ferow)) {
        die(xlt("This patient has no activity."));
    }
    $trans_id = $ferow['id'] ?? 0;
    $encounter = $ferow['encounter'] ?? 0;
                            ...
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    while ($drow = sqlFetchArray($dres)) {
                                        ...
                                        if (!$inv_provider && isset($drow['provider_id']) && $drow['provider_id'] !== null && !empty($arr_users[$drow['provider_id']])) {
                                    ...
                                    if ($inv_encounter) {
                                        ...
                                        $erow = ensureArray($erow);
                                        if (isset($erow['provider_id']) && $erow['provider_id'] !== null) {
