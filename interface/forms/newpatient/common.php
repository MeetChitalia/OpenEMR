...
function normalizeEncounterCommonRow($row)
{
    if (is_object($row) && method_exists($row, 'FetchRow')) {
        if (!property_exists($row, 'EOF') || !$row->EOF) {
            $row = $row->FetchRow();
        } else {
            $row = [];
        }
...
if ($viewmode) {
    ...
    $result = normalizeEncounterCommonRow(sqlQuery("SELECT * FROM form_encounter WHERE id = ?", array($id)));
    ...
    if ($encounter_followup_id) {
        ...
        $followup_enc = normalizeEncounterCommonRow(sqlQuery($q, array($encounter_followup_id)));
        $followup_date = !empty($followup_enc['date']) ? date("m/d/Y", strtotime($followup_enc['date'])) : '';
        $encounter_followup = $followup_enc['encounter'] ?? null;
    }
    // @todo why is this here?
    if ($mode === "followup") {
        $followup_date = !empty($result['date']) ? date("m/d/Y", strtotime($result['date'])) : '';
        $encounter_followup = $result['encounter'] ?? null;
        $result['reason'] = '';
...
<head>
    ...
    if (!$viewmode) {
        ...
        while ($override = sqlFetchArray($q_events)) {
            ...
            $q_enc = normalizeEncounterCommonRow(sqlQuery($q, array($pid, $encnow, $override['pc_aid'])));
    ...
    if (is_object($user_facility)) {
        $user_facility = (array) $user_facility;
    } elseif (!is_array($user_facility)) {
        $user_facility = [];
    }
                        ...
                        <div class="col-sm <?php displayOption('enc_enable_referring_provider');?>">
                            <div class="form-group">
                                ...
                                if ($viewmode && !empty($result["referring_provider_id"])) {
                                    ...
                                    $patientRefProvider = getPatientData($pid, "ref_providerID");
                                    if (is_object($patientRefProvider)) {
                                        $patientRefProvider = (array) $patientRefProvider;
                                    } elseif (!is_array($patientRefProvider)) {
                                        $patientRefProvider = [];
                                    }
                                    $MBO->genReferringProviderSelect('referring_provider_id', '-- ' . xl("Please Select") . ' --', $patientRefProvider['ref_providerID'] ?? '');
                                } ?>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-sm <?php echo ($GLOBALS['hide_billing_widget'] != 1) ?: 'd-none'; ?>">
                            <div class="form-group">
                                ...
                                if (!empty($default_bill_fac_override)) {
                                    ...
                                    if (isset($user_facility['billing_location']) && $user_facility['billing_location'] == '1') {
                        ...
                        <div class="col-sm <?php echo ($GLOBALS['enable_group_therapy'] == 1) ?: 'd-none'; ?>">
                            <div class="form-group">
                                ...
                                if ($viewmode && in_array($result['pc_catid'], $therapyGroupCategories)) {
                                    $selectedGroup = getGroup($result['external_id']);
                                    if (is_object($selectedGroup)) {
                                        $selectedGroup = (array) $selectedGroup;
                                    } elseif (!is_array($selectedGroup)) {
                                        $selectedGroup = [];
                                    }
                                    $selectedGroupName = $selectedGroup['group_name'] ?? '';
                                }
                                ?>
                                <input type='text' name='form_group' class='form-control' id="form_group" placeholder='<?php echo xla('Click to select'); ?>' value='<?php echo attr($selectedGroupName); ?>' onclick='sel_group()' title='<?php echo xla('Click to select group'); ?>' readonly />
                                    ...
                                    <select multiple name='issues[]' class='form-control' title='<?php echo xla('Hold down [Ctrl] for multiple selections or to unselect'); ?>' size='4'>
                                        ...
                                        while ($irow = sqlFetchArray($ires)) {
                                            ...
                                            if ($viewmode) {
                                                ...
                                                if (is_object($perow) && method_exists($perow, 'FetchRow')) {
                                                    $perow = $perow->FetchRow();
                                                } elseif (!is_array($perow)) {
                                                    $perow = [];
                                                }
                                                if (!empty($perow['count'])) {
                                                    echo " selected";
                                                }
                                            } else {
...
<script>
    ...
    // Search for an encounter from today
        ...
        // Depending on the DB wrapper version this can come back as either
        // an array row or an ADORecordSet object.
        if (is_object($erow) && method_exists($erow, 'FetchRow')) {
            if (!property_exists($erow, 'EOF') || !$erow->EOF) {
                $erow = $erow->FetchRow();
            } else {
                $erow = [];
            }
        } elseif (!is_array($erow)) {
            $erow = [];
        }
