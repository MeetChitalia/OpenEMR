...
$facility = is_array($facilityresult) ? ($facilityresult['name'] ?? '') : '';
...
if ($mode == 'new') {
    ...
    // Temporarily disable sensitivity check to allow all users to update encounters
    ...
    $datepart = "";
    $sqlBindArray = array();
    // Temporarily disable date authorization check to allow all users to modify encounter dates
    // if (AclMain::aclCheckCore('encounters', 'date_a')) {
    //     $datepart = "date = ?, ";
    //     $sqlBindArray[] = $date;
    // }
    // Always allow date modification for now
    $datepart = "date = ?, ";
    $sqlBindArray[] = $date;
    array_push(
        $sqlBindArray,
