...
$criticalClasses = [
    'OpenEMR\Common\Logging\EventAuditLogger' => '/src/Common/Logging/EventAuditLogger.php',
    'OpenEMR\Common\Acl\AclMain' => '/src/Common/Acl/AclMain.php',
    'OpenEMR\Common\Csrf\CsrfUtils' => '/src/Common/Csrf/CsrfUtils.php',
    'OpenEMR\Common\Twig\TwigContainer' => '/src/Common/Twig/TwigContainer.php',
    'OpenEMR\Core\Header' => '/src/Core/Header.php'
];

// Load inventory audit system classes
$inventoryClasses = [
    'InventoryAuditLogger' => '/library/InventoryAuditLogger.class.php',
    'EnhancedInventory' => '/library/EnhancedInventory.class.php'
];

foreach ($inventoryClasses as $className => $filePath) {
    if (!class_exists($className)) {
        $classFile = $webserver_root . $filePath;
...
if (!file_exists($webserver_root . "/interface/modules/")) {
    ...
    try {
        ...
        error_log(errorLogEscape(
            "Module bootstrap failed: " . get_class($ex) . ": " . $ex->getMessage() .
            " in " . $ex->getFile() . ":" . $ex->getLine() . "\n" . $ex->getTraceAsString()
        ));
        $GLOBALS['modules_application'] = null;
    }
}
