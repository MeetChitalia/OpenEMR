...
function addOrDeleteColumn($layout_id, $field_id, $add = true)
{
    ...
    $tmp = sqlQueryNoLog("SHOW COLUMNS FROM `" . escape_table_name($tablename) . "` LIKE ?", array($field_id));
...
function renameColumn($layout_id, $old_field_id, $new_field_id)
{
    ...
    $colarr = sqlQueryNoLog("SHOW COLUMNS FROM `" . escape_table_name($tablename) . "` LIKE ?", array($old_field_id));
    if (empty($colarr)) {
        // Error, old name does not exist.
        return 2;
    }
    // Make sure new column does not exist.
    $tmp = sqlQueryNoLog("SHOW COLUMNS FROM `" . escape_table_name($tablename) . "` LIKE ?", array($new_field_id));
    if (!empty($tmp)) {
