...
error_log("edit_globals.php: Starting execution");
                                ...
                                if (!$userMode || in_array($grpname, $USER_SPECIFIC_TABS)) {
                                    ...
                                    foreach ($grparr as $fldid => $fldarr) {
                                        if (!$userMode || in_array($fldid, $USER_SPECIFIC_GLOBALS)) {
                                            ...
                                            if ($userMode) {
                                                    $userSettingRes = sqlStatement("SELECT * FROM user_settings WHERE setting_user=? AND setting_label=?", array($_SESSION['authUserID'],"global:" . $fldid));
                                                    $userSettingArray = sqlFetchArray($userSettingRes);
