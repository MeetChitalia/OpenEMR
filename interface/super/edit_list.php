...
function listChecksum($list_id)
{
    if ($list_id == 'feesheet') {
        ...
        $row = sqlQueryNoLog(
            ...
            "list_id = '" . add_escape_custom($list_id) . "'"
        );
    }
...
if ((($_POST['formaction'] ?? '') == 'save') && $list_id && $alertmsg == '') {
    ...
    $row = sqlQueryNoLog("SELECT max(seq) AS maxseq FROM list_options WHERE list_id= 'lists'");
    $dup_cnt = sqlQueryNoLog("SELECT count(seq) as validate FROM list_options WHERE list_id= 'lists' AND option_id = ?", array($newlistID))['validate'];
    if ((int)$dup_cnt === 0) {
...
function getCodeDescriptions($codes)
{
    ...
    foreach ($arrcodes as $codestring) {
        ...
        $desc = '';
        if ($code_type == 'PROD') {
            $row = sqlQueryNoLog("SELECT name FROM drugs WHERE drug_id = ?", array($code));
            $desc = "$code:$selector " . $row['name'];
        } else {
            $row = sqlQueryNoLog("SELECT code_text FROM codes WHERE " .
                "code_type = ? AND " .
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbar-list">
                ...
                <div class="form-inline my-2 my-lg-0 navbar-left">
                    <select name='list_id' class="form-control select-dropdown" id="list_id" onchange="this.form.submit();">
                        <?php
                        /*
...
<script>
    ...
    $(function () {
        ...
        var SaveChanges = function () {
            ...
            $("#formaction").val("save");
            console.log("formaction set to: " + $("#formaction").val());
