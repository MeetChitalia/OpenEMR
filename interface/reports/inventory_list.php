...
$selectedFacilityScopeId = getSelectedFacilityScopeId();
...
function write_report_line(&$row)
{
    ...
    if ($form_details == 1) { // facility details
        $query = "SELECT " .
            ...
            "WHERE " .
            "s.drug_id = ? AND " .
            "COALESCE(lo.option_value, di.facility_id) = ? AND " .
    ...
    $extrabind = $fwbind;
    if ($form_details == 1) {
        $extracond = "AND COALESCE(lo.option_value, di.facility_id) = ?";
...
if ($selectedFacilityScopeId > 0) {
    $form_facility = $selectedFacilityScopeId;
}
...
$fwbind = array();
if ($form_facility) {
    $fwcond .= " AND COALESCE(lo.option_value, di.facility_id) = ?";
...
if ($form_details == 1) {
    ...
    $query = "SELECT d.*, SUM(di.on_hand) AS on_hand, COALESCE(lo.option_value, di.facility_id) AS option_value, fac.name AS facname, " .
        ...
        "LEFT JOIN list_options AS lo ON lo.list_id = 'warehouse' AND " .
        "lo.option_id = di.warehouse_id AND lo.activity = 1 " .
        "LEFT JOIN facility AS fac ON fac.id = COALESCE(lo.option_value, di.facility_id) " .
        "LEFT JOIN product_warehouse AS pw ON pw.pw_drug_id = d.drug_id AND " .
        "pw.pw_warehouse = di.warehouse_id " .
        "WHERE 1 = 1 $fwcond $actcond " .
        "GROUP BY d.name, d.drug_id, COALESCE(lo.option_value, di.facility_id) ORDER BY $orderby, COALESCE(lo.option_value, di.facility_id)";
...
<table>
 <tr>
  ...
  <td class='text' align='right'>
    ...
    //
    $query = "SELECT id, name FROM facility";
    $binds = [];
    if ($selectedFacilityScopeId > 0) {
        $query .= " WHERE id = ?";
        $binds[] = $selectedFacilityScopeId;
    }
    $query .= " ORDER BY name";
    $fres = sqlStatement($query, $binds);
    echo "   <select name='form_facility' onchange='facchanged()'" . ($selectedFacilityScopeId > 0 ? " disabled='disabled'" : '') . ">\n";
    if ($selectedFacilityScopeId <= 0) {
    ...
    echo "   </select>&nbsp;\n";
    if ($selectedFacilityScopeId > 0) {
        echo "   <input type='hidden' name='form_facility' value='" . attr($selectedFacilityScopeId) . "' />\n";
    }
