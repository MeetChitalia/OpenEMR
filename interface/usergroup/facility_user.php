...
if (isset($_POST["mode"]) && $_POST["mode"] == "facility_user_id" && isset($_POST["user_id"]) && isset($_POST["fac_id"])) {
    ...
    while ($frow = sqlFetchArray($fres)) {
        ...
        $result = sqlStatement("SELECT `id` FROM `facility_user_ids` WHERE `uid` = ? AND `facility_id` = ? AND `field_id` =?", array($_POST["user_id"], $_POST["fac_id"], $frow['field_id']));
        $entry_id = sqlFetchArray($result);
        if (empty($entry_id)) {
            // Insert new entry
                        ...
                        while ($user = sqlFetchArray($u_res)) {
                            foreach ($f_arr as $facility) { ?>
                                <tr>
                                    ...
                                    <?php
                                    foreach ($l_arr as $layout_entry) {
                                        $result = sqlStatement("SELECT `field_value` FROM `facility_user_ids` " .
                                                               "WHERE `uid` = ? AND `facility_id` = ? AND `field_id` = ?", array($user['id'],$facility['id'],$layout_entry['field_id']));
                                        $entry_data = sqlFetchArray($result);
