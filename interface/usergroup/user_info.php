...
function update_password()
{
    ...
    // For temporary password users, skip current password validation
    curPass = '';
    <?php } ?>
    ...
    $.post("user_info_ajax.php",
        ...
        function(data)
        {
            ...
            <?php if (isset($_SESSION['force_password_change']) && $_SESSION['force_password_change'] === true) { ?>
            // If password change was successful, redirect to main screen
            if (data.indexOf('successful') !== -1) {
                setTimeout(function() {
                    window.location.href = webroot + '/interface/main/tabs/main.php';
                }, 2000);
            }
            <?php } ?>
        }
...
<body class="body_top" onload="<?php if (isset($_SESSION['force_password_change']) && $_SESSION['force_password_change'] === true) { ?>document.forms[0].newPass.focus()<?php } else { ?>document.forms[0].curPass.focus()<?php } ?>">
                    ...
                    <div class="form-group">
                        <div class='col-sm-10 offset-sm-2'>
                            <div class="alert alert-info">
                                <?php echo xlt('You are required to change your temporary password. Please enter a new password below.'); ?>
                            </div>
                        </div>
                    </div>
                    <?php } ?>
