...
// Force the desired name for the login page
$openemr_name = 'JACtrac';
...
$logoarea = "py-1 px-2 py-md-2 px-md-3 order-1 text-center";
$formarea = "py-3 px-2 p-sm-5 bg-white order-2";
$loginrow = "row login-row shadow-lg align-items-center m-0 my-sm-5";
...
// Handle AJAX request for user facilities
if (isset($_POST['action']) && $_POST['action'] === 'get_user_facilities' && isset($_POST['username'])) {
    ...
    if (!empty($username)) {
        // Get user ID and check if user is administrator
        $user_query = "SELECT id, facility_id FROM users WHERE username = ? AND active = 1";
        ...
        if ($user_result) {
            ...
            if ($user_data) {
                ...
                $default_facility_id = $user_data['facility_id'];
                
                // Check if user is in Administrators group
                // Using simplified query that should work with OpenEMR's gacl structure
                $admin_check_query = "SELECT COUNT(*) as admin_count 
                                     FROM gacl_aro a
                                     JOIN gacl_groups_aro_map m ON a.id = m.aro_id
                                     JOIN gacl_aro_groups g ON m.group_id = g.id
                                     WHERE g.value = 'admin' AND a.value = ? AND a.section_value = 'users'";
                $admin_result = sqlQuery($admin_check_query, array($username));
                $is_administrator = false;
                
                if ($admin_result) {
                    $admin_data = sqlFetchArray($admin_result);
                    $is_administrator = (isset($admin_data['admin_count']) && $admin_data['admin_count'] > 0);
                }
                
                // If administrator, get all facilities
                if ($is_administrator) {
                    $facilityService = new FacilityService();
                    $all_facilities = $facilityService->getAllFacility();
                    
                    foreach ($all_facilities as $facility) {
                        $user_facilities[] = array(
                            'id' => $facility['id'],
                            'name' => $facility['name']
                        );
                    }
                } else {
                    // Get all facilities assigned to this user
                    $facilities_query = "SELECT DISTINCT uf.facility_id, f.name, f.id 
                                        FROM users_facility uf 
                                        LEFT JOIN facility f ON uf.facility_id = f.id 
                                        WHERE uf.tablename = 'users' AND uf.table_id = ?";
                    $facilities_result = sqlStatement($facilities_query, array($user_id));
                    
                    while ($facility = sqlFetchArray($facilities_result)) {
                        $user_facilities[] = array(
                    ...
                    // If no facilities found in users_facility table, use default facility
                    if (empty($user_facilities) && $default_facility_id) {
                        $default_facility_query = "SELECT id, name FROM facility WHERE id = ?";
                        $default_facility_result = sqlQuery($default_facility_query, array($default_facility_id));
                        if ($default_facility_result) {
                            $default_facility_data = sqlFetchArray($default_facility_result);
                            if ($default_facility_data) {
                                $user_facilities[] = array(
                                    'id' => $default_facility_data['id'],
                                    'name' => $default_facility_data['name']
                                );
                            }
                        }
                    }
    ...
    echo json_encode($user_facilities);
    exit;
}
...
$page_data = [
    ...
    'primaryLogoSrc' => '<img src="' . $GLOBALS['webroot'] . '/public/images/JACtrac.jpg" alt="JACtrac" class="img-fluid" style="max-height:200px;width:auto;">',
    'logocode' => $logocode,
    ...
    // Add attempts left
    'loginAttemptsLeft' => isset($_SESSION['login_attempts_left']) ? $_SESSION['login_attempts_left'] : null,
];
// Clear the session value after passing it
unset($_SESSION['login_attempts_left']);
echo $t->render("login/login_core.html.twig", $page_data);
