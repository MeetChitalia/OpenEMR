...
function existingFinderGetSelectedFacilityId(): int
{
    return isset($_SESSION['facilityId']) ? (int) $_SESSION['facilityId'] : 0;
}

function existingFinderBuildSelectedFacilityWhereClause(int $facilityId): string
{
    return " WHERE (facility_id = ? OR FIND_IN_SET(?, REPLACE(COALESCE(care_team_facility, ''), '|', ',')) > 0)";
}

function existingFinderCalculatePatientBalance($pid)
{
...
function existingFinderCalculatePatientLifetimePayments($pid)
{
    $receipts = sqlQuery(
    ...
    $posPayments = sqlQuery(
        "SELECT SUM(amount) AS total_payments
         ...
         WHERE pid = ?
           AND amount > 0
           AND transaction_type IN ('external_payment', 'payment', 'credit_payment', 'purchase', 'purchase_and_dispens', 'purchase_and_alterna', 'dispense')",
        array($pid)
    );
    ...
    return max($receiptTotal, $posPaymentTotal);
}

function buildExistingFinderFallbackRowsHtml()
{
    $facilityFilter = "";
    $facilityParams = array();
    $selectedFacilityId = existingFinderGetSelectedFacilityId();
    ...
    if ($selectedFacilityId > 0) {
        $facilityFilter = existingFinderBuildSelectedFacilityWhereClause($selectedFacilityId);
    ...
    while ($row = sqlFetchArray($rows)) {
        ...
        $backdateUrl = $GLOBALS['webroot'] . '/interface/pos/backdate_pos_screen.php?pid=' . urlencode((string) $row['pid']);
        $backdateHtml = '<a class="btn btn-info btn-sm pay-now-btn backdate-pos-btn" data-pid="' . attr($row['pid']) . '" href="' . attr($backdateUrl) . '" onclick="event.stopPropagation();"><i class="fa fa-calendar"></i> Backdate</a>';
...
$finderFallbackRowsHtml = buildExistingFinderFallbackRowsHtml();

$popup = empty($_REQUEST['popup']) ? 0 : 1;
$searchAny = empty($_GET['search_any']) ? "" : $_GET['search_any'];
$backdateMode = empty($_GET['backdate_mode']) ? 0 : 1;
...
// // Add Balance column
// $header .= "   <th>" . xlt('Balance') . "</th>\n";
// $header0 .= "   <td style='display: none;'><input type='text' size='20' value='' class='form-control search_init' placeholder='" . xla("Search by Balance") . "'/></td>\n";
// if ($coljson) {
//     $coljson .= ", ";
// }
// $coljson .= "{\"sName\": \"balance\", \"mRender\": formatBalance}";
// if ($orderjson) {
//     $orderjson .= ", ";
// }
...
<html>
<head>
    <?php Header::setupHeader(['datatables', 'datatables-colreorder', 'datatables-dt', 'datatables-bs', 'opener', 'datetime-picker']); ?>
...
<body>
            ...
            <div class="unified-search-container">
                <div class="search-controls">
                    <div class="search-input-wrapper">
                        ...
                        <input type="text" id="unified_search" class="form-control" placeholder="<?php echo xla('Search by Name, DOB, or Mobile Phone...'); ?>" autocomplete="off" />
                    </div>
                ...
                <div id="dynamic">
                    <div class="table-responsive">
                        <table class="table table-modern display" id="pt_table">
                            ...
                            <tbody>
                                <?php if (!empty($finderFallbackRowsHtml)) { ?>
                                    <?php echo $finderFallbackRowsHtml; ?>
                                <?php } else { ?>
                                <tr>
    ...
    <script>
      $(function() {
        ...
        $('.mytopdiv form').addClass('p-3 bg-light border-top');
      });
    </script>
    ...
    <script>
        ...
        var backdateMode = <?php echo (int) $backdateMode; ?>;
        ...
        function applyUnifiedSearch() {
            ...
            if (window.patientFinderTable) {
                window.patientFinderTable.search(currentUnifiedSearch).draw();
                return true;
            }
...
<script>
    ...
    $(function () {
            ...
            "ajax": function(data, callback) {
                $.ajax({
                    ...
                    data: {
                        ...
                        draw: data.draw || 1,
                        start: data.start || 0,
                        length: data.length || <?php echo empty($GLOBALS['gbl_pt_list_page_size']) ? '10' : (int) $GLOBALS['gbl_pt_list_page_size']; ?>,
                        search: {
                            value: currentUnifiedSearch
                        },
                        order: data.order || []
                    },
                    success: function(response) {
                        callback({
                            draw: parseInt(response.draw || data.draw || 1, 10),
                            recordsTotal: parseInt(response.recordsTotal || 0, 10),
                            recordsFiltered: parseInt(response.recordsFiltered || 0, 10),
        ...
        $('#pt_table').on('click', 'tbody tr', function (event) {
            ...
            try {
                ...
                if (balanceColumnIndex >= 0) {
                    ...
                    // console.log('Balance column not found'); // Removed console.log
                }
            } catch (error) {
                // console.error('Error getting balance:', error); // Removed console.error
                balance = 0;
            }
        ...
        function openDispenseTrackingModal(pid) {
            ...
            try {
                ...
                var posUrl = getFinderWebroot() + '/interface/pos/backdate_pos_screen.php?pid=' + encodeURIComponent(pid) + '&dispense_tracking=1';
        ...
        $('#pt_table').on('click', '.pay-now-btn, .backdate-pos-btn', function (event) {
            event.preventDefault();
            event.stopPropagation();
    ...
    function formatBalance(data, type, full) {
        if (type == 'display') {
            var balance = parseFloat(data);
            if (balance > 0) {
                return '<span class="text-danger font-weight-bold">$' + balance.toFixed(2) + '</span>';
            } else {
  ...
  function formatPayNow(data, type, full, meta) {
    ...
    try {
        if (meta && meta.row !== undefined) {
            var rowNode = $('#pt_table').DataTable().row(meta.row).node();
            if (rowNode && rowNode.id && rowNode.id.indexOf('pid_') === 0) {
    ...
    var url = getFinderWebroot() + '/interface/pos/backdate_pos_screen.php?pid=' + encodeURIComponent(pid);
    return '<a class="btn btn-info btn-sm pay-now-btn backdate-pos-btn" data-pid="' + pid + '" href="' + url + '" onclick="event.stopPropagation();">' +
        '<i class="fa fa-calendar"></i> Backdate</a>';
}
