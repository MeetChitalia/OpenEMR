...
// Simple search query
$totalPaidSql = "GREATEST(
    0,
    (
        CASE
            WHEN EXISTS(
                SELECT 1
                FROM pos_receipts AS prx
                WHERE prx.pid = patient_data.pid
                  AND prx.amount > 0
            )
            THEN COALESCE(
                (SELECT SUM(pr.amount)
                 FROM pos_receipts AS pr
                 WHERE pr.pid = patient_data.pid
                   AND pr.amount > 0),
                0
            )
            ELSE COALESCE(
    ...
    COALESCE(
        (SELECT SUM(vt.amount)
         FROM pos_transactions AS vt
         WHERE vt.pid = patient_data.pid
           AND vt.transaction_type = 'void'
           AND vt.amount < 0),
        0
    ) -
    COALESCE(
        (SELECT SUM(rf.refund_amount)
         FROM pos_refunds AS rf
         WHERE rf.pid = patient_data.pid
           AND rf.refund_type = 'payment'
...
if (!empty($searchTerm)) {
    ...
    $query = "SELECT pid, lname, fname, mname, phone_cell, phone_home, pubpid, DOB, $totalPaidSql
              FROM patient_data 
              WHERE (lname LIKE ? 
                 OR fname LIKE ? 
                 OR mname LIKE ? 
    ...
    $dobPattern = dateSearch($searchTerm);
    ...
    $params = array_merge(
        array(
            ...
            $searchPattern, // fname  
            $searchPattern, // mname
            $searchPattern, // phone_cell
            $searchPattern, // phone_home
            $searchPattern, // pubpid
            $dobPattern     // DOB
        ),
        $facilityParams,
        array(
...
while ($row = sqlFetchArray($res)) {
    ...
    $totalPaid = isset($row['total_paid']) ? (float) $row['total_paid'] : calculatePatientLifetimePayments($row['pid']);
    $arow[] = $totalPaid;

    // Pay Now column (keep passing actual balance for POS routing)
    $balance = calculatePatientBalance($row['pid']);
