...
function Patient($doc, $r, $pid)
{
    ...
    $result = sqlStatement("select *, DATE_FORMAT(DOB,'%Y%m%d') AS date_of_birth from patient_data where pid=?", array($pid));
    $patient_data = sqlFetchArray($result);
...
function OutsidePrescription($doc, $r, $pid, $prescid)
{
    global $msg;
    if ($prescid) {
        $result = sqlStatement("SELECT p.note,p.dosage,p.substitute,p.per_refill,p.form,p.route,p.size,p.interval,p.drug,l1.title AS title1,l2.title AS title2,l3.title AS title3,l4.title AS title4,p.id AS prescid,
