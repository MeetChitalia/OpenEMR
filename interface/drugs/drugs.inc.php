...
function formatDrugInventoryNumber($value, $decimals = 2)
{
    if ($value === null || $value === '') {
        return '';
    }
...
function formatDrugInventoryDisplayNumber($value, $decimals = 3)
{
    if ($value === null || $value === '') {
        return '';
    }
    ...
    $number = (float) $value;
    $formatted = number_format($number, $decimals, '.', '');
    return ((float) $formatted) == 0.0 ? number_format(0, $decimals, '.', '') : $formatted;
}

function parseDrugInventoryNumber($value)
{
    if ($value === null || $value === '') {
        return null;
    }
...
function getLiquidInventoryDefinition($drug)
{
    ...
    $ml_per_vial = parseDrugInventoryNumber($size);
    $mg_per_ml = null;
    ...
    if (
        ...
        $mg_amount = (float) $matches[1];
        $volume_amount = empty($matches[2]) ? 1.0 : (float) $matches[2];
        if ($volume_amount > 0) {
            $mg_per_ml = $mg_amount / $volume_amount;
        }
    }

    $is_ml_form = ($form === 'ml');

    $has_liquid_indicators = (
        $is_ml_form ||
    ...
    $is_liquid = $is_ml_form || ($has_liquid_indicators && $ml_per_vial !== null && $ml_per_vial > 0);
    $has_complete_liquid_strength = $mg_per_ml !== null && $mg_per_ml > 0;

    return array(
        'is_ml_form' => $is_ml_form,
        'is_liquid' => $is_liquid,
        'has_complete_strength' => $has_complete_liquid_strength,
        'ml_per_vial' => $ml_per_vial,
        'mg_per_ml' => $mg_per_ml,
        'mg_per_vial' => ($is_liquid && $has_complete_liquid_strength) ? ($ml_per_vial * $mg_per_ml) : null,
        'size_display' => $is_liquid ? formatDrugInventoryNumber($ml_per_vial) . ' mL per vial' : $size,
        'strength_display' => ($is_liquid && $has_complete_liquid_strength)
            ? formatDrugInventoryNumber($mg_per_ml) . ' mg / mL'
            : $unit,
    );
}

function convertLiquidVialsToMg($drug, $vial_quantity)
{
    $definition = getLiquidInventoryDefinition($drug);
    if (empty($definition['is_liquid']) || empty($definition['mg_per_vial'])) {
        return (float) $vial_quantity;
    }

    return (float) $vial_quantity * (float) $definition['mg_per_vial'];
}

function formatDrugInventoryQuantity($drug, $quantity)
{
    $definition = getLiquidInventoryDefinition($drug);
    if (!empty($definition['is_liquid'])) {
        return formatDrugInventoryNumber($quantity) . ' mg';
    }

    return formatDrugInventoryNumber($quantity);
}

function ensureDrugInventoryMetadataSchema()
{
    ...
    $row = sqlQueryNoLog("SHOW COLUMNS FROM drugs LIKE 'vial_quantity'");
    if (empty($row)) {
        sqlStatement("ALTER TABLE drugs ADD COLUMN vial_quantity DECIMAL(12,2) NOT NULL DEFAULT 0 AFTER route");
    }
}
...
function sellDrug(
  ...
  // error_log("quantity = '$quantity'"); // debugging
    ...
    if (empty($default_warehouse)) {
        ...
        $rowuser = sqlQueryNoLog("SELECT default_warehouse FROM users WHERE username = ?", array($user));
        $default_warehouse = $rowuser['default_warehouse'];
    }

  // Get relevant options for this product.
    $rowdrug = sqlQueryNoLog("SELECT allow_combining, reorder_point, name, dispensable " .
...
function isUserRestricted($userid = 0)
{
    ...
    $countrow = sqlQueryNoLog("SELECT count(*) AS count FROM users_facility WHERE " .
