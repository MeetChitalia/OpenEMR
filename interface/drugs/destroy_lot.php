...
require_once("$srcdir/InventoryAuditLogger.class.php");
...
if ($_POST['form_save']) {
    ...
    // Log the lot destruction to audit system
    $logger = new InventoryAuditLogger($_SESSION['authUserID']);
    $logger->logLotActivity(
        $drug_id, 
        $lot_id, 
        'lot_delete', 
        null, 
        array(
            'destroy_date' => $_POST['form_date'],
            'destroy_method' => $_POST['form_method'],
            'destroy_witness' => $_POST['form_witness'],
            'destroy_notes' => $_POST['form_notes']
        ), 
        'Lot destroyed via destroy_lot.php'
    );
 ...
 $row = sqlQueryNoLog("SELECT * FROM drug_inventory WHERE drug_id = ? " .
