...
function isSnomedSpanish()
{
    ...
    $res = sqlStatement("SELECT `revision_version` FROM `standardized_tables_track` WHERE `name` = 'SNOMED' ORDER BY `id` DESC");
    $sql = sqlFetchArray($res);
  ...
  // ensure $desc_detail is set properly
    ...
    if (!empty($codes)) {
        ...
        foreach ($relcodes as $codestring) {
            ...
            if ($crow && is_array($crow) && !empty($crow[$desc_detail])) {
                if ($code_text) {
                    $code_text .= '; ';
                }
