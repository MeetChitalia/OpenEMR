...
// Helper function to safely get values from sqlQuery results
function getSqlValue($result, $key, $default = '') {
    if (is_object($result) && method_exists($result, 'FetchRow')) {
        $row = $result->FetchRow();
        return ($row && isset($row[$key])) ? $row[$key] : $default;
    } elseif (is_array($result) && isset($result[$key])) {
        return $result[$key];
    }
    return $default;
}
...
function frontPayment($patient_id, $encounter, $method, $source, $amount1, $amount2, $timestamp, $auth = "")
{
    ...
    $encounter_date = '';
    if (is_object($tmprow) && method_exists($tmprow, 'FetchRow')) {
        $tmprow_data = $tmprow->FetchRow();
        if ($tmprow_data && isset($tmprow_data['date'])) {
            $encounter_date = $tmprow_data['date'];
        }
    } elseif (is_array($tmprow) && isset($tmprow['date'])) {
        $encounter_date = $tmprow['date'];
    }
    ...
    $tmprowArray = explode(' ', $encounter_date);
 ...
 //In the screen a drop down of Ins1,Ins2,Ins3,Pat are given.The posting can be done for any level.
    ...
    if ($Affected == 'yes') {
		...
		pid ='" . trim(formData('hidden_patient_code')) . "' and encounter='" . trim(formData("HiddenEncounter$CountRow")) . "'");
            $last_level_closed = getSqlValue($ferow, 'last_level_closed', 0);
              //multiple charges can come.
            if ($last_level_closed < trim(formData("HiddenIns$CountRow"))) {
                ...
                if ((empty($_POST["FollowUp$CountRow"]) || ($_POST["FollowUp$CountRow"] != 'y')) && $tmp !== 0) {
                  ...
                  $ferow = sqlQuery("SELECT date, last_level_closed " .
                    ...
                    "pid = '" . trim(formData('hidden_patient_code')) . "' AND encounter = '" . trim(formData("HiddenEncounter$CountRow")) . "'");
                  $encounter_date = getSqlValue($ferow, 'date', '');
                  $encounter_last_level = getSqlValue($ferow, 'last_level_closed', 0);
                  $date_of_service = substr($encounter_date, 0, 10);
                  $new_payer_type = 0 + $encounter_last_level;
                if ($new_payer_type <= 3 && !empty($encounter_last_level) || $new_payer_type == 0) {
                    ++$new_payer_type;
                }
