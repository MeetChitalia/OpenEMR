...
function generate_select_list(
    ...
    if (!$got_selected && strlen($currvalue ?? '') > 0 && !$multiple) {
        ...
        if (!empty($backup_list) && !empty($lrow) && !empty($lrow['title'])) {
...
{
    $data_type = $frow['data_type'] ?? 1;
    $list_id = $frow['list_id'] ?? '';
    $backup_list = $frow['backup_list'] ?? '';
    $s = '';

    // Defensive check to avoid SQL errors: return immediately if list_id or currvalue is empty for list-based fields
    if (($data_type == 1 || $data_type == 26 || $data_type == 43 || $data_type == 46) && (empty($list_id) || empty($currvalue))) {
        return '<span> <i class="fa fas fa-exclamation-circle ml-1"></i></span>';
    }
    // ... existing code ...
}
...
function display_layout_rows($formtype, $result1, $result2 = '')
{
    ...
    while ($prow = sqlFetchArray($pres)) {
        ...
        while ($frow = sqlFetchArray($fres)) {
            ...
            // Skip guardian fields if patient is 18 or older (for DEM form only)
            if ($formtype == 'DEM' && $this_group == '8' && !shouldShowGuardianFields($GLOBALS['pid'])) {
                continue;
            }
...
function display_layout_tabs($formtype, $result1, $result2 = '')
{
    ...
    while ($prow = sqlFetchArray($pres)) {
        ...
        while ($frow = sqlFetchArray($fres)) {
            ...
            // Skip guardian tab if patient is 18 or older (for DEM form only)
            if ($formtype == 'DEM' && $this_group == '8' && !shouldShowGuardianFields($GLOBALS['pid'])) {
                continue;
            }
...
function display_layout_tabs_data($formtype, $result1, $result2 = '')
{
    ...
    while ($prow = sqlFetchArray($pres)) {
        ...
        while ($frow = sqlFetchArray($fres)) {
            ...
            // Modern card-based group wrapper
            echo "<div class='modern-card mb-4'>\n";
            echo "<div class='modern-title'>" . text(xl_layout_label($grparr[$this_group]['grp_title'])) . "</div>\n";
                    if ($subtitle) {
                echo "<div class='modern-subtitle mb-2'>" . text($subtitle) . "</div>\n";
            }
            // Add modern-grid for tight, responsive layout
            echo "<div class='modern-form-section modern-grid'>\n";

            // Render each field in the group
            while ($group_fields = sqlFetchArray($group_fields_query)) {
                $currvalue = get_layout_form_value($group_fields, '', $result1, $result2);
                $label = xl_layout_label($group_fields['title']);
                    $field_id = 'text_' . $group_fields['field_id'];
                echo "<div class='modern-form-group'>\n";
                echo "  <label for='" . attr($field_id) . "' class='modern-label'>" . text($label) . ":</label>\n";
                echo "  <div class='modern-data' id='" . attr($field_id) . "' data-value='" . attr($currvalue) . "'>";
    ...
    while ($prow = sqlFetchArray($pres)) {
        ...
        while ($frow = sqlFetchArray($fres)) {
            ...
            while ($group_fields = sqlFetchArray($group_fields_query)) {
                ...
                $label = xl_layout_label($group_fields['title']);
                $form_field_id = 'form_' . $field_id;
                
                echo "<div class='modern-form-group'>\n";
                echo "  <label for='" . attr($form_field_id) . "' class='modern-label'>" . text($label) . ":</label>\n";
                echo "  <div class='modern-input-wrapper'>";
                // Add modern styling to form controls
                $group_fields['smallform'] = ' modern-input';
...
function dropdown_facility(
    ...
    $forcedFacilityId = getSelectedFacilityScopeId();
    if ($forcedFacilityId > 0) {
        $selected = (string) $forcedFacilityId;
        $allow_allfacilities = false;
        $allow_unspecified = false;
    }
    ...
    foreach ($fres as $frow) {
        $facility_id = $frow['id'];
        if ($forcedFacilityId > 0 && (int) $facility_id !== $forcedFacilityId) {
            continue;
        }
...
function getSelectedFacilityScopeId(): int
{
    return isset($_SESSION['facilityId']) ? (int) $_SESSION['facilityId'] : 0;
}

function enforceSelectedFacilityScopeForRequest(string $fieldName = 'form_facility'): int
{
    $selectedFacilityId = getSelectedFacilityScopeId();
    if ($selectedFacilityId <= 0) {
        return 0;
    }

    $selectedFacilityValue = (string) $selectedFacilityId;
    $_REQUEST[$fieldName] = $selectedFacilityValue;
    $requestMethod = strtoupper($_SERVER['REQUEST_METHOD'] ?? 'GET');
    if ($requestMethod === 'POST') {
        $_POST[$fieldName] = $selectedFacilityValue;
    } else {
        $_GET[$fieldName] = $selectedFacilityValue;
    }
...
/**
 * Check if guardian fields should be displayed based on patient age
 * @param int $pid Patient ID
 * @return bool True if guardian fields should be shown (patient age < 18)
 */
function shouldShowGuardianFields($pid) {
    // For new patients, check the DOB from form data
    if (empty($pid) || $pid == 0) {
        if (!empty($_POST['form_DOB'])) {
            $dob_date = new DateTime($_POST['form_DOB']);
            $today = new DateTime();
            $age = $today->diff($dob_date)->y;
            return $age < 18;
        }
        // If no DOB available, show guardian fields by default for new patients
        return true;
    }
    // For existing patients, get data from database
...
function display_layout_group_editable($formtype, $group_id, $result1 = [], $result2 = '') {
    global $item_count, $cell_count, $last_group, $CPR, $condition_str, $BS_COL_CLASS;

    $grparr = [];
    getLayoutProperties($formtype, $grparr, '*');
    $TOPCPR = empty($grparr['']['grp_columns']) ? 4 : $grparr['']['grp_columns'];
    $CPR = empty($grparr[$group_id]['grp_columns']) ? $TOPCPR : $grparr[$group_id]['grp_columns'];
    ...
    echo "<div class='modern-card mb-4'>\n";
    echo "<div class='modern-title'>" . text(xl_layout_label($grparr[$group_id]['grp_title'])) . "</div>\n";
    if ($subtitle) {
        echo "<div class='modern-subtitle mb-2'>" . text($subtitle) . "</div>\n";
    }
    echo "<div class='modern-form-section modern-grid'>\n";
    foreach ($fields as $group_fields) {
        ...
        // Special handling for name_history field to make it smaller
        if ($field_id === 'name_history') {
            $group_fields['fld_rows'] = 2; // Reduce to 2 rows instead of default
            $group_fields['fld_length'] = 40; // Reduce width
        }
