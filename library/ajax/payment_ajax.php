...
//===============================================================================
    if ($_POST["ajax_mode"] == "update_sell_price") {//update sell price for inventory
        // Check if user has admin or inventory permissions
        ...
        if ($inventory_id <= 0) {
            echo json_encode(array('status' => 'error', 'message' => 'Invalid inventory ID'));
            die;
        }
        
        if ($sell_price < 0) {
            echo json_encode(array('status' => 'error', 'message' => 'Sell price cannot be negative'));
            die;
        }

        // First check if the inventory record exists and get drug info
        $res = sqlStatement(
            "SELECT di.inventory_id, d.name, di.lot_number 
        ...
        $drug_info = sqlFetchArray($res);
        if (!$drug_info) {
            echo json_encode(array('status' => 'error', 'message' => 'Inventory record not found'));
            die;
        }
        
        // Update the sell price in drug_inventory table
        sqlStatement(
            "UPDATE drug_inventory SET sell_price = ? WHERE inventory_id = ?",
            array($sell_price, $inventory_id)
        );

        // Log the change for audit purposes
        $user = $_SESSION['authUser'] ?? 'unknown';
        $drug_name = $drug_info['name'] ?? 'Unknown';
        $lot_number = $drug_info['lot_number'] ?? 'Unknown';
        
        // Log to audit trail if available
        if (function_exists('auditLog')) {
            auditLog($user, 'inventory_update', 'sell_price', 
                "Updated sell price for drug: $drug_name, lot: $lot_number, new price: $sell_price");
        }
        
        echo json_encode(array(
            'status' => 'success', 
            'message' => 'Sell price updated successfully',
            'formatted_price' => '$' . number_format($sell_price, 2)
        ));
        die;
    }
